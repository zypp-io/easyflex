...
def pull_module(run_params, module_name):
    ...
    for jaar in run_params.years:
        run = True
        parameters = {"rf_decl_jaar": jaar}
        logging.info(f"start import declaraties {jaar}")
        while run:

            operatie = OperatieParameters(
                naam="ds_rf_declaraties_all",
                output_prefix="",
        ...
        if len(data):
            export_module(data, module_name, run_params, jaar)


def export_module(data, module_name, run_params, jaar):

    data = format_data(data)
    data["werkmaatschappij"] = run_params.adm_code
    data["decl_id"] = data["werkmaatschappij"] + " - " + data["rf_decl_idnr"]
    filename = "{}_{}_{}.pkl".format(run_params.adm_code, module_name, jaar)
