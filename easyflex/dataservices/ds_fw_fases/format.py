...
def convert_stapsysteem(df):
    if "fa_stapsysteem" in df.columns:
        waardes = {
        ...
        df["fa_stapsysteem"] = df.fa_stapsysteem.map(
            waardes
...
def convert_to_date(df):
    columns = ["fa_stapeerstedag", "fa_staplaatstedag"]
...
def correct_open_enddate(df):
    ...
    if "fa_staplaatstedag" in df.columns:
        df.loc[df["fa_staplaatstedag"].dt.date == today, "fa_staplaatstedag"] = np.nan
